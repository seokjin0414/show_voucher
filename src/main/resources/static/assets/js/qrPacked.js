...
var qrcode = function () {
    ...
    function k(h, b, e, d, c, a) {
        ...
        this.buildFunctionPattern = function () {
            ...
            for (var e = this.alignmentPatternCenters.length, b = 0; b < e; b++) for (var a = this.alignmentPatternCenters[b] - 2, h = 0; h < e; h++) 0 == b && (0 == h || h == e - 1) || b == e - 1 && 0 == h || c.setRegion(this.alignmentPatternCenters[h] -
                2, a, 5, 5);
    ...
    function Q(h) {
        ...
        this.processFinderPatternInfo = function (b) {
            ...
            if (1 > c) throw"Error";
            var h = this.computeDimension(e, d, b, c), a = k.getProvisionalVersionForDimension(h),
                m = a.DimensionForVersion - 7, f = null;
    ...
    function T(a) {
        ...
        this.readCodewords = function () {
            var b = this.readFormatInformation(), d = this.readVersion(), c = H.forReference(b.DataMask),
            ...
            for (var c = d.buildFunctionPattern(), a = !0, h = Array(d.TotalCodewords), m = 0, f = 0, g = 0, k = b - 1; 0 < k; k -= 2) {
                6 == k && k--;
                for (var x = 0; x < b; x++) for (var v = a ? b - 1 - x : x, t = 0; 2 > t; t++) c.get_Renamed(k - t, v) || (g++, f <<= 1, this.bitMatrix.get_Renamed(k -
    ...
    function S() {
        ...
        this.findFinderPattern = function (a) {
            ...
            for (var h = !1, l = Array(5), m = c - 1; m < e && !h; m += c) {
                ...
                l[2] = 0;
                l[3] = 0;
                for (var f = b = l[4] = 0; f < d; f++) if (a[f + m * g.width]) 1 == (b & 1) && b++, l[b]++; else if (0 == (b & 1)) if (4 == b) if (this.foundPatternCross(l)) {
                    if (b = this.handlePossibleCenter(l, m, f)) c = 2, this.hasSkipped ? h = this.haveMultiplyConfirmedCenters() :
                        (b = this.findRowSkip(), b > l[2] && (m += b - l[2] - c, f = d - 1)); else {
                    ...
                    l[3] = 0;
                    l[4] = 0
                } else l[0] = l[2], l[1] = l[3], l[2] = l[4], l[3] = 1, l[4] = 0, b = 3; else l[++b]++; else l[b]++;
        ...
        this.handlePossibleCenter = function (c, d, b) {
            ...
            if (!isNaN(d)) {
                ...
                for (var a = this.possibleCenters.length,
                         e = 0; e < a; e++) if (this.possibleCenters[e].aboutEquals(c, d, b)) return new M(b, d, c);
                b = new M(b, d, c);
    ...
    var N = [[21522, 0], [20773, 1], [24188, 2], [23371, 3], [17913, 4], [16590, 5], [20375, 6], [19104, 7], [30660, 8], [29427,
            9], [32170, 10], [30877, 11], [26159, 12], [25368, 13], [27713, 14], [26998, 15], [5769, 16], [5054, 17], [7399, 18], [6608, 19], [1890, 20], [597, 21], [3340, 22], [2107, 23], [13663, 24], [12392, 25], [16177, 26], [14854, 27], [9396, 28], [8579, 29], [11994, 30], [11245, 31]],
    ...
    E.rsDecoder = new function (a) {
        ...
        this.decode = function (a, e) {
            ...
            if (!h) for (f = new w(this.field, c), b = this.runEuclideanAlgorithm(this.field.buildMonomial(e, 1), f, e), f = b[1],
        ...
        this.runEuclideanAlgorithm = function (a, e, d) {
            ...
            for (var b = this.field.One, f = this.field.Zero, h = this.field.Zero, g = this.field.One; e.Degree >= Math.floor(d / 2);) {
                ...
                for (f = this.field.inverse(f); e.Degree >= a.Degree && !e.Zero;) {
                    var k = e.Degree - a.Degree, r = this.field.multiply(e.getCoefficient(e.Degree), f),
    ...
    var g = {
        ...
        process: function (a) {
            ...
            if (g.debug) {
                for (var d = 0; d < g.height; d++) for (var c = 0; c < g.width; c++) {
            ...
            e =
                (new Q(e)).detect();
            if (g.debug) {
                for (d = 0; d < e.bits.Height; d++) for (c = 0; c < e.bits.Width; c++) f = 8 * c + 2 * d * g.width * 4, g.imagedata.data[f] = (e.bits.get_Renamed(c, d), 0), g.imagedata.data[f + 1] = (e.bits.get_Renamed(c, d), 0), g.imagedata.data[f + 2] = e.bits.get_Renamed(c, d) ? 255 : 0;
                a.putImageData(g.imagedata, 0, 0)
            }
        ...
        binarize: function (a) {
            for (var b = Array(g.width * g.height), e = 0; e < g.height; e++) for (var d = 0; d < g.width; d++) {
        ...
        getMiddleBrightnessPerArea: function (a) {
            ...
            for (c = 0; 4 > c; c++) for (f =
                                             0; 4 > f; f++) {
                d[f][c][0] = 255;
                for (var h = 0; h < e; h++) for (var m = 0; m < b; m++) {
        ...
        grayScaleToBitmap: function (a) {
            for (var b = g.getMiddleBrightnessPerArea(a), e = b.length, d = Math.floor(g.width / e), c = Math.floor(g.height / e), f = new ArrayBuffer(g.width * g.height), f = new Uint8Array(f), h = 0; h < e; h++) for (var m = 0; m < e; m++) for (var k =
                0; k < c; k++) for (var n = 0; n < d; n++) f[d * m + n + (c * h + k) * g.width] = a[d * m + n + (c * h + k) * g.width] < b[m][h] ? !0 : !1;
            return f
        },
        grayscale: function () {
            for (var a = new ArrayBuffer(g.width * g.height), a = new Uint8Array(a), b = 0; b < g.height; b++) for (var e = 0; e < g.width; e++) {
